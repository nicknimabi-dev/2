<template>
	<div class="addEdit-block">
		...
		>
			<template >
				<el-form-item class="input" v-if="type!='info'" label="活动编号" prop="activitynumber" >
					<el-input v-model="ruleForm.activitynumber" placeholder="活动编号" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-else-if="ruleForm.activitynumber" label="活动编号" prop="activitynumber" >
					<el-input v-model="ruleForm.activitynumber" placeholder="活动编号" readonly></el-input>
				</el-form-item>
				...
				<el-form-item class="select" v-if="type!='info'"  label="活动类型" prop="activitytype" >
					<el-select :disabled="ro.activitytype" v-model="ruleForm.activitytype" placeholder="请选择活动类型" >
						<el-option
				...
				<el-form-item v-else class="input" label="活动类型" prop="activitytype" >
					<el-input v-model="ruleForm.activitytype"
						placeholder="活动类型" readonly></el-input>
				</el-form-item>
				<el-form-item class="date" v-if="type!='info'" label="活动时间" prop="activitytime" >
					<el-date-picker
						value-format="yyyy-MM-dd HH:mm:ss"
						v-model="ruleForm.activitytime" 
						type="datetime"
						:readonly="ro.activitytime"
						:picker-options="activitytimePickerOptions"
						placeholder="活动时间"
					></el-date-picker>
				</el-form-item>
				<el-form-item class="input" v-else-if="ruleForm.activitytime" label="活动时间" prop="activitytime" >
				...
				<el-form-item class="input" v-if="type!='info'"  label="参与人数" prop="activityquota" >
					<el-input v-model.number="ruleForm.activityquota" placeholder="参与人数" clearable  :readonly="ro.activityquota"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="参与人数" prop="activityquota" >
					<el-input v-model="ruleForm.activityquota" placeholder="参与人数" readonly></el-input>
				</el-form-item>
			</template>
			<el-form-item v-if="type!='info'"  label="活动路线" prop="activityroute" >
				<editor 
					style="min-width: 200px; max-width: 600px;"
					v-model="ruleForm.activityroute" 
					class="editor"
					myQuillEditor="activityroute"
					action="file/upload">
				</editor>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.activityroute" label="活动路线" prop="activityroute" >
				<span class="text ql-snow ql-editor" v-html="ruleForm.activityroute"></span>
			</el-form-item>
		...
		methods: {
			...
			init(id,type) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						if(o=='activitynumber'){
							this.ruleForm.activitynumber = obj[o];
							this.ro.activitynumber = true;
							continue;
						}
						if(o=='activityname'){
						...
						if(o=='activityroute'){
							this.ruleForm.activityroute = obj[o];
							this.ro.activityroute = true;
							continue;
						}
						if(o=='clicktime'){
					...
					this.ruleForm.activityquota = Number('0'); 				}
				// 获取用户信息
				this.$http({
				...
				this.$http({
					url: `option/activitytype/activitytype`,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
						this.activitytypeOptions = data.data;
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.activityroute = this.ruleForm.activityroute.replace(reg,'../../../springbootdti6c7gx/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			// 提交
			async onSubmit() {
					if(this.ruleForm.activitynumber) {
						this.ruleForm.activitynumber = String(this.ruleForm.activitynumber)
					}
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `walkingactivity/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.walkingactivityCrossAddOrUpdateFlag = false;
											this.parent.search();
