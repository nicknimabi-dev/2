<template>
	<div class="main-content" :style='{"width":"100%","padding":"20px 30px","fontSize":"15px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"0px solid #fff","padding":"10px","margin":"0","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"0px 0","flexWrap":"wrap","background":"none","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('activityregistration','活动报名人数')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						活动报名人数
					</el-button>
					<el-button class="btn18" v-if="isAuth('activityregistration','月报名人数')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						月报名人数
					</el-button>
					<el-button class="btn18" v-if="isAuth('activityregistration','活动类型统计')" type="success" @click="chartDialog3()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						活动类型统计
					</el-button>
				</el-row>
			...
			<div :style='{"border":"0px solid #fff","padding":"20px 20px 120px","margin":"0px auto","color":"#000","background":"url(http://codegen.caihongy.cn/20241117/5628c107d57245e1bbab7c77b946c1cf.jpg) repeat-x center bottom,#fff","width":"100%","fontSize":"14px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="reasonforregistration"
						label="报名原因">
						<template slot-scope="scope">
							{{scope.row.reasonforregistration}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="useraccount"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="edit" v-if=" isAuth('activityregistration','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								修改
							</el-button>
		...
		<el-dialog
			...
			width="800">
			<div id="activitynameChart1" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
		...
		<el-dialog
			...
			width="800">
			<div id="activityquotaChart2" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
		...
		<el-dialog
			...
			width="800">
			<div id="activitytypeChart3" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
	...
	export default {
		...
		methods: {
			...
			chartDialog1() {
				...
				this.$nextTick(()=>{
					var activitynameChart1 = echarts.init(document.getElementById("activitynameChart1"),'bee-inspired');
					this.$http({
						url: "activityregistration/group/activityname",
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
							...
							for(let i=0;i<res.length;i++){
								...
								xAxis.push(res[i].activityname);
								yAxis.push(parseFloat((res[i].total)));
								pArray.push({
									value: parseFloat((res[i].total)),
									name: res[i].activityname
								})
							}
							var option = {};
							...
							// 使用刚指定的配置项和数据显示图表。
							activitynameChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								activitynameChart1.resize();
							};
						}else{
							this.$message({
								message: data.msg,
			...
			chartDialog2() {
				...
				this.$nextTick(()=>{
					...
					var activityquotaChart2 = echarts.init(document.getElementById("activityquotaChart2"),'bee-inspired');
					this.$http({
						url: `activityregistration/value/registrationtime/activityquota/月`,
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
							...
							for(let i=0;i<res.length;i++){
								...
								xAxis.push(res[i].registrationtime);
								yAxis.push(parseFloat((res[i].total)));
								pArray.push({
									value: parseFloat((res[i].total)),
									name: res[i].registrationtime
								})
							}
							var option = {};
							...
							// 使用刚指定的配置项和数据显示图表。
							activityquotaChart2.setOption(option);
							//根据窗口的大小变动图表
							window.onresize = function() {
								activityquotaChart2.resize();
							};
						}else{
							this.$message({
								message: data.msg,
			...
			chartDialog3() {
				...
				this.$nextTick(()=>{
					...
					var activitytypeChart3 = echarts.init(document.getElementById("activitytypeChart3"),'bee-inspired');
					this.$http({
						url: "activityregistration/group/activitytype",
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
							...
							for(let i=0;i<res.length;i++){
								...
								xAxis.push(res[i].activitytype);
								yAxis.push(parseFloat((res[i].total)));
								pArray.push({
									value: parseFloat((res[i].total)),
									name: res[i].activitytype
								})
							}
							var option = {};
							let titleObj = this.pie.title
							titleObj.text = '活动类型统计'
							...
							// 使用刚指定的配置项和数据显示图表。
							activitytypeChart3.setOption(option);
							//根据窗口的大小变动图表
							window.onresize = function() {
								activitytypeChart3.resize();
							};
						}else{
							this.$message({
								message: data.msg,
				...
				this.$refs["shBatchForm"].validate(valid => {
					if(valid){
						this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
							...
							this.$http({
								url: "activityregistration/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
								method: "post",
								data: this.batchIds
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
